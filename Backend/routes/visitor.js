...
router.put('/update/:visitorID', async (req, res) => {
    try {
        ...
        const updateEmp = await visitorSchema.findOneAndUpdate({visitorID:req.params.visitorID},{$set: req.body},{ new: true });
...
try {
    const deletevisitor= await visitorSchema.findOne(req.params.visitorID);
    if(!deletevisitor) {
        return res.status(200).json("visitor not found")
    }

    const dvis= await visitorSchema.deleteOne(req.params.visitorID)
